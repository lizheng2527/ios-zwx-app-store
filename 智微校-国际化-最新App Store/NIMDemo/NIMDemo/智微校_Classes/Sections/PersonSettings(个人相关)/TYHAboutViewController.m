...
//    UIImageView *logoImageView = [[UIImageView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/2 - 40,[UIScreen mainScreen].bounds.size.height/2 - 120, 80, 80)];
//    logoImageView.image = [UIImage imageNamed:@"Identifier"];
    UIImageView *logoImg = [UIImageView new];
    [self.view addSubview:logoImg];
    
    logoImg.sd_layout.leftSpaceToView(self.view, ([UIScreen mainScreen].bounds.size.width -70)/2).topSpaceToView(self.view, 170).heightIs(70).widthIs(70);
    logoImg.image = [UIImage imageNamed:@"Identifier"];
   
    UILabel *appName = [UILabel new];
    [self.view addSubview:appName];
    appName.sd_layout.leftSpaceToView(self.view, 40).rightSpaceToView(self.view, 40).topSpaceToView(logoImg, 20).heightIs(30);
    appName.text = @"智微校";
    appName.textAlignment = NSTextAlignmentCenter;
    appName.font = [UIFont boldSystemFontOfSize:18];
    
    
    UILabel *versionName = [UILabel new];
    [self.view addSubview:versionName];
    NSString *versionString1 = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"];
    versionName.sd_layout.leftSpaceToView(self.view, 40).rightSpaceToView(self.view, 40).topSpaceToView(appName, 1).heightIs(30);
    versionName.text = [NSString stringWithFormat:@"Version %@",versionString1] ;
    versionName.textAlignment = NSTextAlignmentCenter;
    
    UIButton *btn = [UIButton new];
    [self.view addSubview:btn];
    btn.sd_layout.leftSpaceToView(self.view, 100).rightSpaceToView(self.view, 100).bottomSpaceToView(self.view, 50) .heightIs(20);
    [btn setTitle:@"隐私声明" forState:(UIControlStateNormal)];
    btn.titleLabel.font = [UIFont boldSystemFontOfSize:13];
    btn.titleLabel.textAlignment = NSTextAlignmentCenter;
    [btn setTitleColor:[UIColor colorWithRed:65 /255.0 green:105 / 255.0 blue:185/255.0 alpha:1] forState:UIControlStateNormal];
    [btn addTarget:self action:@selector(checkPrivateAction) forControlEvents:UIControlEventTouchUpInside];
    
    UILabel *companyLabel = [UILabel new];
    [self.view addSubview:companyLabel];
    companyLabel.sd_layout.leftSpaceToView(self.view, 40).rightSpaceToView(self.view, 40).topSpaceToView(btn, 10).heightIs(15);
    companyLabel.textAlignment = NSTextAlignmentCenter;
    companyLabel.text = @"北京中电和讯科技有限公司 版权所有";
...
//    UILabel *appLabel = [[UILabel alloc]initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/2 - 80,[UIScreen mainScreen].bounds.size.height/2 - 120, 160, 40)];
    
    
//    appLabel.sd_layout.leftSpaceToView(self.view, 30).rightEqualToView(<#UIView *toView#>)
...
//
////    versionLabel.text = [NSString stringWithFormat:@"%@ V%@",NSLocalizedString(@"", nil),versionString];
...
//    checkUpdateBtn.frame = CGRectMake(versionLabel.frame.size.width, 0,60, 20);
...
//
//    //分享标记
////    shareTrueDownloadURL = @"https://oa.zdhx-edu.com/Mobile/index.html";
//
...
////    }
////
//    http://www.zdhx-edu.com/hexun/tpl/img/zd-new-banner2.png
////    else
//    {
//                shareTrueDownloadURL = @"https://oa.zdhx-edu.com/Mobile/index.html";
//                shareTrueCodeImage = @"智微校AppStore分享";
//
//        //如果上传App Store
////        shareTrueDownloadURL = @"https://itunes.apple.com/cn/app/id1461040258";
