...
- (void)registerPushService
{
    if (@available(iOS 11.0, *))
    {
        ...
        [center requestAuthorizationWithOptions:(UNAuthorizationOptionBadge | UNAuthorizationOptionSound | UNAuthorizationOptionAlert) completionHandler:^(BOOL granted, NSError * _Nullable error) {
            if (!granted)
            {
                [[UIApplication sharedApplication].keyWindow makeToast:@"请开启推送功能否则无法收到应用内的消息提醒" duration:2.0 position:CSToastPositionCenter];
            }
        }];
    }
    else
...
-(void)initBaiduMap
{
    ...
    BOOL ret = [_mapManager start:@"dpk699Po5k1FAGGmd4ZTbk3443Iyo2hA" generalDelegate:self];
    if (!ret) {
